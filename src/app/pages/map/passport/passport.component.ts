...
export class PassportComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  //выбираем название, собираем поля паспорта,
  // задаём что можно редактировать
  ngOnInit() {
    if (this.objectType === 'well') {
      ...
      this.passportData = [
        { label: 'ID', value: (this.objectData.id || 1).toString() },
        {
          label: 'Координаты',
          value: `[${
            this.objectData.position ? this.objectData.position[0] : 54.406709
          }, ${
            this.objectData.position ? this.objectData.position[1] : 50.793813
          }]`,
        },
        ...
        {
          label: 'Глубина',
          value: this.objectData.depth != null ? this.objectData.depth : '',
        },
        {
          label: 'Диаметр',
          value:
            this.objectData.diameter != null ? this.objectData.diameter : '',
        },
      ...
      this.passportData = [
        ...
        {
          label: 'Потребители',
          value: this.objectData.userConnections.length.toString(),
        },
        {
          label: 'Диаметр',
          value: this.objectData.diameter
            ? this.objectData.diameter.toString() + ' мм'
            : '',
        },
      ...
      this.passportData = [
        ...
        {
          label: 'Вершина 1 (индекс)',
          value: `${fromIndex} [${from[0]}, ${from[1]}]`,
        },
        {
          label: 'Вершина 2 (индекс)',
          value: `${toIndex} [${to[0]}, ${to[1]}]`,
        },
      ...
      this.passportData = [
        ...
        { label: 'Адрес', value: this.objectData.address || '' },
        {
          ...
          value:
            this.objectData.geodeticMark != null
              ? this.objectData.geodeticMark
              : '',
        },
        {
          label: 'Диаметр выходного отверстия',
          value:
            this.objectData.outletDiameter != null
              ? this.objectData.outletDiameter
              : '',
        },
        { label: 'Имя', value: this.objectData.name || '' },
        { label: 'Категория', value: this.objectData.category || '' },
        {
          label: 'Минимальный напор воды',
          value:
            this.objectData.minPressure != null
              ? this.objectData.minPressure
              : '',
        },
        {
          label: 'Напор',
          value:
            this.objectData.pressure != null ? this.objectData.pressure : '',
        },
        {
          label: 'Относительный расход воды',
          value:
            this.objectData.relativeFlow != null
              ? this.objectData.relativeFlow
              : '',
        },
        {
          label: 'Полный напор',
          value:
            this.objectData.fullPressure != null
              ? this.objectData.fullPressure
              : '',
        },
        {
          label: 'Расчетный расход воды в будний день',
          value:
            this.objectData.flowWeekday != null
              ? this.objectData.flowWeekday
              : '',
        },
        {
          label: 'Расчетный расход воды в воскресенье',
          value:
            this.objectData.flowSunday != null
              ? this.objectData.flowSunday
              : '',
        },
        {
          label: 'Расчетный расход воды в праздники',
          value:
            this.objectData.flowHoliday != null
              ? this.objectData.flowHoliday
              : '',
        },
        {
          label: 'Расчетный расход воды в субботу',
          value:
            this.objectData.flowSaturday != null
              ? this.objectData.flowSaturday
              : '',
        },
        {
          label: 'Расчётный расход воды',
          value:
            this.objectData.flowCalculated != null
              ? this.objectData.flowCalculated
              : '',
        },
        {
          label: 'Способ задания потребителя',
          value: this.objectData.userMethod || '',
        },
        {
          label: 'Текущий расход воды',
          value:
            this.objectData.currentFlow != null
              ? this.objectData.currentFlow
              : '',
        },
        {
          label: 'Уровень воды',
          value:
            this.objectData.waterLevel != null
              ? this.objectData.waterLevel
              : '',
        },
      ];
      this.editableKeys = [
        ...
        'Полный напор',
        'Расчетный расход воды в будний день',
        'Расчетный расход воды в воскресенье',
        'Расчетный расход воды в праздники',
        'Расчетный расход воды в субботу',
      ...
      this.fieldTypes = {
        Адрес: 'string',
        'Геодезическая отметка': 'number',
        'Диаметр выходного отверстия': 'number',
        Имя: 'string',
        Категория: 'string',
        'Минимальный напор воды': 'number',
        Напор: 'number',
        'Относительный расход воды': 'number',
        'Полный напор': 'number',
        'Расчетный расход воды в будний день': 'number',
        'Расчетный расход воды в воскресенье': 'number',
        'Расчетный расход воды в праздники': 'number',
        'Расчетный расход воды в субботу': 'number',
        'Расчётный расход воды': 'number',
        'Способ задания потребителя': 'string',
        'Текущий расход воды': 'number',
        'Уровень воды': 'number',
      };
    } else if (
      this.objectType === 'capture' ||
      this.objectType === 'pump' ||
      this.objectType === 'reservoir' ||
      ...
      this.passportData = [
        ...
        {
          label: 'Координаты',
          value: this.objectData.position
            ? this.objectData.position.join(', ')
            : '',
        },
      ];
    ...
    console.log(
      `ngOnInit: objectId=${this.objectId}, objectName=${this.objectName}, objectType=${this.objectType}, originalZIndex=${this.originalZIndex}, originalHeight=${this.originalHeight}`
    );
  }

  //настраиваем размеры и позицию окна на экране,
  // подписывается на клик по документу
  ngAfterViewInit() {
    this.updatePassportHeight();
    this.updateVisibleRows();
    this.passportBlock.nativeElement.style.left = `${
      10 + (this.objectId || 0) * 20
    }px`;
    this.passportBlock.nativeElement.style.top = `${
      10 + (this.objectId || 0) * 20
    }px`;
    this.passportBlock.nativeElement.style.zIndex = this.originalZIndex;
    console.log(
      `ngAfterViewInit: objectId=${this.objectId}, initial left=${this.passportBlock.nativeElement.style.left}, top=${this.passportBlock.nativeElement.style.top}, zIndex=${this.passportBlock.nativeElement.style.zIndex}, data-object-id=${this.passportBlock.nativeElement.dataset['objectId']}`
    );

    document.addEventListener('click', this.handleDocumentClick);
  }

  // чистим обработчики событий и обновляем список свёрнутых паспортов
  ngOnDestroy() {
    document.removeEventListener('click', this.handleDocumentClick);
    if (this.isMinimized && this.objectId !== null && !isNaN(this.objectId)) {
      PassportComponent.minimizedOrder =
        PassportComponent.minimizedOrder.filter((id) => id !== this.objectId);
      this.updateMinimizedPositions();
      console.log(
        `ngOnDestroy: Removed objectId=${this.objectId} from minimizedOrder, new minimizedOrder=`,
        PassportComponent.minimizedOrder
      );
    }
  ...
  // клик по свёрнутому паспорту: поднимаем его на передний план
  onPassportClick(event: MouseEvent) {
    if (!this.isMinimized) return;
    event.stopPropagation();

    const passport = this.passportBlock.nativeElement;
    const allPassports = document.querySelectorAll(
      '.passport-block'
    ...
    if (
      ...
      const prevPassport = Array.from(allPassports).find(
        (p) =>
          p.dataset['objectId'] ===
          PassportComponent.lastActivePassportId?.toString()
      );
      if (
        prevPassport &&
        PassportComponent.minimizedOrder.includes(
          PassportComponent.lastActivePassportId
        )
      ) {
        const index = PassportComponent.minimizedOrder.indexOf(
          PassportComponent.lastActivePassportId
        );
        const originalZ = 1000 + index;
        prevPassport.style.zIndex = `${originalZ}`;
        console.log(
          `onPassportClick: Restored zIndex of previous active passport id=${PassportComponent.lastActivePassportId} to ${originalZ}`
        );
      }
    }
    ...
    PassportComponent.lastActivePassportId = this.objectId;
    console.log(
      `onPassportClick: objectId=${this.objectId}, new zIndex=${passport.style.zIndex}, lastActivePassportId=${PassportComponent.lastActivePassportId}`
    );
  }
  ...
  private handleDocumentClick = (event: MouseEvent) => {
    ...
    if (this.isMinimized && !passport.contains(target)) {
      if (
        this.objectId === PassportComponent.lastActivePassportId &&
        this.objectId !== null &&
        PassportComponent.minimizedOrder.includes(this.objectId)
      ) {
        const index = PassportComponent.minimizedOrder.indexOf(this.objectId);
        passport.style.zIndex = `${1000 + index}`;
        PassportComponent.lastActivePassportId = null;
        console.log(
          `handleDocumentClick: Restored zIndex of objectId=${this.objectId} to ${passport.style.zIndex}, lastActivePassportId=${PassportComponent.lastActivePassportId}`
        );
      }
    }
  };

  // сворачивает или разворачивает паспорт
  toggleMinimize(event?: MouseEvent) {
    event?.stopPropagation();
    console.log(
      `toggleMinimize: objectId=${this.objectId}, current isMinimized=${this.isMinimized}, minimizedOrder before=`,
      PassportComponent.minimizedOrder
    );
    ...
    if (this.isMinimized) {
      ...
      if (this.objectId !== null && !isNaN(this.objectId)) {
        PassportComponent.minimizedOrder = [
          ...PassportComponent.minimizedOrder.filter(
            (id) => id !== this.objectId
          ),
          this.objectId,
        ];
        ...
        console.warn(
          `toggleMinimize: Invalid objectId=${this.objectId}, not adding to minimizedOrder`
        );
      }
      ...
      const index =
        this.objectId !== null && !isNaN(this.objectId)
          ? PassportComponent.minimizedOrder.indexOf(this.objectId)
          : -1;
      const leftOffset = index >= 0 ? 10 + index * 200 : 10;
      passport.style.left = `${leftOffset}px`;
      passport.style.top = containerRect.height
        ? `${containerRect.height - 40}px`
        : 'auto';
      ...
      console.log(
        `Minimized: objectId=${
          this.objectId
        }, index=${index}, left=${leftOffset}px, top=${
          containerRect.height - 40
        }px, zIndex=${passport.style.zIndex}, styles=${
          passport.style.cssText
        }, minimizedOrder=`,
        PassportComponent.minimizedOrder
      );
    } else {
      if (this.objectId !== null && !isNaN(this.objectId)) {
        ...
        console.warn(
          `toggleMinimize: Invalid objectId=${this.objectId}, not removing from minimizedOrder`
        );
      }
      ...
      console.log(
        `Maximized: objectId=${this.objectId}, styles=${passport.style.cssText}, zIndex=${passport.style.zIndex}, minimizedOrder=`,
        PassportComponent.minimizedOrder
      );
    }
  }

  // пересчитывает позиции всех свёрнутых паспортов
  private updateMinimizedPositions() {
    console.log(
      'updateMinimizedPositions: Starting, minimizedOrder=',
      PassportComponent.minimizedOrder
    );
    const container = document.querySelector(
      '.passport-container'
    ...
    setTimeout(() => {
      allPassports.forEach((p, idx) => {
        ...
        const id =
          rawId && !isNaN(parseInt(rawId, 10)) ? parseInt(rawId, 10) : null;
        const isMinimized = p.classList.contains('minimized');
        console.log(
          `updateMinimizedPositions: passport[${idx}] rawId=${rawId}, id=${id}, isMinimized=${isMinimized}, in minimizedOrder=${
            id !== null && PassportComponent.minimizedOrder.includes(id)
          }`
        );
        ...
        if (
          id !== null &&
          isMinimized &&
          PassportComponent.minimizedOrder.includes(id)
        ) {
          ...
          console.log(
            `updateMinimizedPositions: Set passport id=${id} to left=${leftOffset}px, top=${
              containerRect.height - 40
            }px, zIndex=${zIndex}, index=${index}`
          );
        } else {
          console.log(
            `updateMinimizedPositions: Skipping passport id=${id}, reason: ${
              id === null
                ? 'invalid id'
                : !isMinimized
                ? 'not minimized'
                : 'not in minimizedOrder'
            }`
          );
        }
      });
  ...
  // закрывает паспорт и убирает его из списка
  closePassport() {
    if (this.objectId !== null && !isNaN(this.objectId)) {
      PassportComponent.minimizedOrder =
        PassportComponent.minimizedOrder.filter((id) => id !== this.objectId);
      ...
      this.updateMinimizedPositions();
      console.log(
        `closePassport: Removed objectId=${this.objectId} from minimizedOrder, emitted passportClosed, new minimizedOrder=`,
        PassportComponent.minimizedOrder
      );
    } else {
      console.warn(
        `closePassport: Invalid objectId=${this.objectId}, not removing from minimizedOrder or emitting event`
      );
    }
    if (this.objectId === PassportComponent.lastActivePassportId) {
      PassportComponent.lastActivePassportId = null;
  ...
  // процесс перетаскивания (мышь двигается)
  private onDrag = (event: MouseEvent) => {
    ...
    newLeft = Math.max(
      0,
      Math.min(newLeft, containerRect.width - passportRect.width)
    );
    newTop = Math.max(
      0,
      Math.min(newTop, containerRect.height - passportRect.height)
    );
  ...
  // начало изменения ширины левой колонки
  ...
  // процесс изменения ширины левой колонки
  private onResizeColumn = (event: MouseEvent) => {
    ...
    const newWidth = Math.max(
      200,
      Math.min(maxLeftColumnWidth, this.startWidth + deltaX)
    );
    this.leftColumn.nativeElement.style.width = `${newWidth}px`;
  };

  // завершение изменения ширины левой колонки
  ...
  // начало изменения общей ширины паспорта
  ...
  // процесс изменения общей ширины
  ...
  // начало изменения высоты паспорта
  startResizingHeight(event: MouseEvent) {
    ...
    if (!this.isFixed) {
      this.previousHeight =
        this.passportBlock.nativeElement.style.height || this.originalHeight;
    }
  ...
  // фиксирует паспорт справа или возвращает обратно
  toggleFixPassport() {
    if (this.isMinimized) return;
    const passport = this.passportBlock.nativeElement;
    const container = document.querySelector(
    ...
    if (!this.isFixed) {
      ...
      container.style.width =
        this.previousContainerStyles.width || 'calc(100% - 20px)';
      container.style.height =
        this.previousContainerStyles.height || 'calc(100vh - 140px)';
      container.style.maxHeight =
        this.previousContainerStyles.maxHeight || 'calc(100% - 20px)';

      passport.style.width = this.previousWidth;
      passport.style.height = this.wasMinimizedDuringFix
        ? this.originalHeight
        : this.previousHeight;
      passport.style.left = this.previousLeft;
      passport.style.top = this.previousTop;
  ...
  // считает максимально возможную высоту паспорта
  ...
  // корректирует высоту паспорта при изменении
  ...
  // обрезает список строк по высоте окна
  private updateVisibleRows() {
    ...
    const availableHeight =
      passportHeight - headerHeight - footerHeight - padding;
  ...
  // вычисляет длину отрезка (для труб)
  ...
  // сохраняет изменения в полях паспорта
  onSave() {
  ...
  // отменяет редактирование и возвращает старые данные
  onCancel() {
    this.passportData.forEach((item) => {
      this.editFields[item.label] = item.value;
    });
  ...
  // переводит название поля (метку) в ключ объекта
